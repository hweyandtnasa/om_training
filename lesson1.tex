...
Let $x \equiv [x_1,x_2,...,x_m]^T$ be a vector of inputs and let $y \equiv [y_1,y_2,...,y_n]^T$ be a vector of outputs. An \textbf{explicit function} is one that defines $y$ by an equation of the form
\begin{equation}
y \equiv F(x)
\end{equation}
where $F$ is some function. An output of an explicit function is called an \textit{explicit output}. Notice how the equation above is actually a \textit{system} of equations, with exactly one equation per explicit output. 

For example, the function with real input $x$ and real output $y$ defined by the equation $y = x^2$ is explicit, since the output $y$ is isolated on the left-hand side and the right-hand side is strictly an expression of the input $x$.

Any non-explicit function that is otherwise well-defined is called an \textbf{implicit function}. In the most general sense, the defining equations of implicit functions can be written as a system of the form
\begin{equation}
R(x,y) \equiv 0
\end{equation}
for some function $R$. Such an $R$ is called a \textbf{residual function}, and any particular value $r$ of $R$ is called a \textbf{residual}. These equations \textit{implicitly define} $y$, which is the vector of \textit{implicit outputs}. There is one residual, and thus one equation, for each implicit output. 

For example, the function with real input $x$ and real output $y$ defined by the equation $y^2 = x$ is implicit, since $y$ is not isolated.

Notice how the function defined by $y^2 = x$ is \textit{not} the same as that defined by $y = \pm \sqrt{x}$, at the very least because the former is implicit and the latter is explicit. Thus, whether a function is explicit or implicit depends upon the particular formulation of its defining equations. It is standard form to write explicit and implicit systems in the forms given above.

An important thing to remember is that any explicit function can be rewritten as an implicit function, but not all implicit functions can be rewritten as explicit functions! For instance, since OpenMDAO works fundamentally with implicit functions, it automatically converts user-defined explicit functions $y \equiv F(x)$ into implicit functions by subtracting the right-hand side from the left:
\begin{equation}
R(x,y) \equiv y - F(x) \equiv 0
\end{equation}
...
These implicit functions are defined by \textit{residual equations}, such as 
    \begin{equation} 
    r \equiv R(x, y) \equiv 0     
    \label{eq:res_eq} 
    \end{equation}
where $x \equiv [x_1,x_2,...,x_m]^T$ is the input vector and $y \equiv [y_1,y_2,...,y_n]^T$ is the implicit output vector of the function.
