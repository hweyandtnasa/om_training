...
\noindent Though formal optimization theory is quite old, what we think of today as "optimization" is a fairly young discipline that emerged with the development of general purpose computing. 
    ...
    \begin{itemize}[itemsep=1.2em, label={}, itemindent=-2.9em]
        \item 1740: British mathematician Thomas Simpson describes the modern form of a multivariate \textit{Newton's Method} in ``Essays on Several Curious and Useful Subjects, in Speculative and Mix'd Mathematicks'', pointing out its usefulness for solving optimization problems. 
        \item 1857: J. W. Gibbs shows that chemical equilibrium is attained when the energy is a minimum.
        \item 1939: William Karush derives the necessary conditions for the inequality constrained problem in his Masters thesis. Harold Kuhn and Albert Tucker rediscover these conditions and publish their seminal  
        ...
        \item 1975: John Holland proposes the first genetic algorithm.
    ...
    \section{Why do we need optimization and what does ``gradient-based'' mean?}
        ...
        \subsubsection{Gradient-based}
            ...
            Gradient-based methods are the only class of optimization algorithms that can handle thousands, tens of thousands, or even millions of design variables. 
...
There are two types of functions, with respect to how their defining equations relate inputs to outputs: explicit functions and implicit functions. 

An \textbf{explicit function} is one in which each output is isolated on one side of its own defining equation, being equated to an expression strictly of inputs on the other side. For example, the function with real input $x$ and real output $y$ defined by the equation $y = x^2$ is explicit, since the output $y$ is isolated on the left-hand side and the right-hand side is strictly an expression of the input $x$. To put it another way, the key property of an explicit function is that all outputs are separated from all inputs and each other in the defining equations. An output from an explicit function is called an \textit{explicit output}.

Any well-defined function that is not explicit is called an \textbf{implicit function}. In an implicit function, there is at least one designated output variable "mixed in" with other inputs and/or outputs---that is, there is at least one output that does not have its own defining equation in which it is isolated on one side and equated to an expression strictly of inputs. For example, if we were to flip the roles of $x$ and $y$ in the previous example so that $y$ were \textit{input} and $x$ were \textit{output}, the resulting function would be implicit---with defining equation $x^2 = y$---because, even though output $x$ does has its own defining equation, it is not in fact \textit{isolated} on either side, instead nested within the squaring operator. An output from an implicit function is called an \textit{implicit output}.

Notice how the function defined by $x^2 = y$ (with input $y$ and output $x$) is \textit{not} the same as that defined by $x = \sqrt{y}$ (with the same input and output), at the very least because the former is implicit and the latter is explicit. Thus, whether a function is explicit or implicit depends upon the particular formulation of its defining equations.

Speaking of particular formulations of defining equations, there are standards that ought to be adhered to when laying out the equations for explicit and implicit functions. Let $x \equiv [x_1,x_2,...,x_m]^T$ be a vector of inputs and let $y \equiv [y_1,y_2,...,y_n]^T$ be a vector of outputs. It is convention to write the defining equations for an explicit function as a system of the form
\begin{equation}
    y = F(x)
\end{equation}
where $F$ is some function.
For an implicit function, all variables are to be moved to the same side of the defining equations so that we can write the equations as
\begin{equation}
    R(x,y) = 0
\end{equation}
for some function $R$. Such an $R$ is called a \textbf{residual function}, and any particular value $r$ of $R$ is called a \textbf{residual}.

\subsection{Implicit Systems of Equations}
...
An \textbf{implicit system of equations} is a collection of inter-related implicit functions that must be solved simultaneously. 
These implicit functions are defined by \textit{residual equations}, such as 
    \begin{equation} 
    r = R(x, y) = 0     
    \label{eq:res_eq} 
    \end{equation}
where $x=[x_1,x_2,...,x_m]^T$ is the input vector and $y=[y_1,y_2,...,y_n]^T$ is the implicit output vector of the function.

In any well-defined function, implicit or explicit, there is a single defining equation for each output. Therefore, the length $n$ of $y$ will always be the same as the length of $R$, i.e. the number of individual residual function components.

Since implicit systems do not have the benefit of defining their outputs as explicit functions of inputs, optimizers must solve implicit systems by "driving the residual to 0"---namely, by somehow computing $x,y$ such that Equation \eqref{eq:res_eq} holds. With this difficulty in mind, why should we even bother working with implicit systems? 

First of all, we generally have no choice! While all explicit functions $y=F(x)$ can be reformulated as implicit functions $R(x,y) \equiv y-F(x) = 0$, not all implicit functions can be made explicit.
Consider, for instance, the explicit function of scalar input $x$ with scalar output $y$ given by
\begin{equation*}
    y = F(x) \equiv 3x^2 + 2x
\end{equation*}
This is an explicit function, but it can be reformulated as an implicit function by subtracting the right-hand side from the output on the left:
\begin{equation*}
    R(x,y) \equiv y - F(x) = y - (3x^2 + 2x) = 0
\end{equation*}
However, if instead we had an implicit function
\begin{equation*}
    y e^y = x^3y - 1
\end{equation*}
then there is no way to reformulate this as an explicit function since y cannot be solved for in the equation. This means that, given an input x, we cannot simply "plug and chug" into the equation to compute the output y! A solver routine is required.

The second reason for using implicit systems of equations is that, as we will see later, the implicit form of a function is generally more useful for systems analysis and optimization. Much of this usefulness stems from the way gradients are computed for implicit functions, which is very different from how you would do so for explicit functions. We'll revisit this later when discussing gradient calculations.

To illustrate how these concepts apply to aeronautics, let us look at the Mach-Area relationship for compressible, isentropic flow: 
\begin{equation}
...
Equation~\eqref{eq:mach_area} is an \textit{explicit} function that computes the area ratio $A/A^*$ directly, given any value for $MN$. 
...
We're discussing this subtlety because internally and out of sight from the users, \textit{OpenMDAO represents all functions in their implicit form}. 
...
\subsection{Disguising Implicit Functions as Explicit Functions}
...
\end{align}
Equation~\eqref{eq:Newton_method} is called \textbf{Newton's method}. 
Because we dropped the higher order terms, the solution to this linear system is only an approximation to the roots of $R(x,y)$. 
However, we can iteratively solve Equation~\eqref{eq:Newton_method} and apply the $\Delta y$ update until we reach a sufficient level of accuracy. 
Also, since the terms we disregarded are of $\mathcal{O}( \Delta y^2)$, we know that Newton's method should exhibit second-order convergence when near the true root. In other words, the error of the approximation decreases quadratically as our step size decreases.
...
\begin{equation}
    ...
    \right] 
\end{equation}
That is a fairly large equation, which highlights the reality that hand computing derivatives can end up being a lot of work for large systems of arbitrary equations. 
So it is very common to approximate the partial derivative Jacobian using finite-differences. 
We'll talk more about the various ways you can get the derivatives you need in further sections, 
but the key here is to remember how Newton's method works. 
...
\section{Introduction to the Theory Behind Gradient-Based Optimization}
    ...
    There are an infinite number of solutions for $f$, corresponding to the infinite number of values that $x_1$ can take. 
    ...
    Since this function is parabolic and opening upward, we know that there is one and only one minimum for all values of $x_1$. 
    ...
    Now let's look at a parabolic function of two variables: 
    \begin{equation}
        f = F(x_1,x_2) = (x_1 - 3)^2 + x_1 x_2 + (x_2 + 4.0)^2 - 3 
    \end{equation}

    Since it is still parabolic and opening upward, we know that there is still only one minimum. 
    $f$ is still a scalar, but now the input is two scalars $x_1,x_2$.
    ...
    \end{align}
    Now we can solve this (implicit) system of equations for $x_1$ and $x_2$ to find the minimum of the original objective function. Notice how Equation~\eqref{eq:linear_deriv} can be re-arranged into a linear system of equations: 
    \begin{equation}
        \begin{bmatrix}
        2,1 \\
        1,2
    ...
    \end{equation}
    We made it cubic by changing the cross term $x_1 x_2$ into $x_1^2 x_2$, and so now the system of equations to find the minimum will be: 
    \begin{align}
    ...
    For this particular problem, perhaps you can find the roots by hand using substitution---although, generally, things are not that simple. 

    What if your objective is a cubic function of 1000 variables $x \equiv [x_1,x_2,...,x_{1000}]^T$? 
    \begin{equation}
        f = F(x_1,x_2, ..., x_{1000}) = F(x) 
    \end{equation}
    Now, to find the minima we will have to solve a nonlinear system of 1000 equations: 
    \begin{equation} 
    r = R(x_1, x_2, ...,  x_{1000}) = R(x) = \frac{\partial f}{\partial x} = 0 
    \end{equation}
    ...
    \begin{equation}
         \Delta x = - \left[\frac{\partial R}{\partial x} \right]^{-1} r 
    \end{equation}
    where $\left[\frac{\partial R}{\partial x} \right]$ is a matrix of partial derivatives computed about some $x^*$, $\left[\Delta x\right]$. The quantity $r$ is the \textit{vector} of residual values computed at $x^*$, and $\Delta x$ is the update that should be applied to $x^*$ for the next iteration. 

    Note that we now need $\frac{\partial R}{\partial x}$, but $R$ was itself defined as a partial derivative of $f$ with respect to $x$. 
    So that means that we now need second-derivatives with respect to $f$: 
    \begin{equation}
         \left[\frac{\partial R}{\partial x} \right] =  \left[\frac{\partial^2 f}{\partial x^2} \right]
    \end{equation}
    The matrix $\frac{\partial^2 f}{\partial x^2}$ is called the \textbf{Hessian matrix}, or simply ''Hessian'', of $f$. It is square and symmetric, with the number of rows and columns equal to the length of $x$.
    
    And so, for an unconstrained optimization, we can create a well-defined problem (same number of unknowns as equations to constrain them) by taking derivatives of the objective function with respect to the design variables in the above manner. 

    If the system of equations we get by taking those derivatives is nonlinear, then we need to use Newton's method or some other nonlinear solver to iteratively solve for roots that will minimize the objective.
    If $x$ is a vector of length 1000, The Hessian of $f$ will be $1000 \times 1000$, 
    ...
    Some optimization algorithms, called \textbf{quasi-Newton methods}, approximate the Hessian using only first-order derivative information.

    Thus, while you most likely do not need to explicitly compute the Hessian, you still need to compute the first-derivatives of the objective function with respect to the design variables. 
    ...
    Nonetheless, even if an optimizer does not directly solve this system of nonlinear equations, it is still searching for solutions to it. 
    Hence, we have gone over this material so that you can understand how an optimizer sees the problem you are posing to it. 

    \section{Taking Derivatives of Implicit Outputs}
