...
\section{Introduction to the Theory Behind Gradient-Based Optimization}
    ...
    \section{Taking derivative of outputs of mixed implicit/explicit functions}
    ...
    The optimizer is going to need $df/dx$, but how do we compute that? 
    ...
    Just as we discussed before, the computational scaling for this approach is bad if $x$ grows to be a large vector (if you have a fine mesh for your FEA for example). 
    Lets assume we're willing to pay the computational cost though. 
    ...
    We could use complex-step to get better accuracy, if the code can handle complex arithmetic. 
    That would be even more expensive than first order FD though, and not all engineering codes can handle complex arithmetic either. 
    So our goal is to find another way that is both more computationally efficient than FD and complex-step, and a more accurate than FD. 

    %Derive the direct method 
    % Now we have something that is at least more accurate, if not more efficient
